...
parameters = np.load("parameters_cat_2017-11-04_19-01-10.npy").all()
...
print("Training set Accuracy:"+str(get_accuracy(np.array(train_pred > 0.8).astype(int).T, train_y.T)))
test_pred, caches = dnn.forward_propagation(test_x, parameters, activation_dims)
print("Test set Accuracy:"+str(get_accuracy(np.array(test_pred > 0.8).astype(int).T, test_y.T)))
